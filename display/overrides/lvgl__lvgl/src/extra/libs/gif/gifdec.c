#pragma message "[OVERRIDES] Using patched gifdec.c"
#include "gifdec.h"
...
/* --- Colour helpers ------------------------------------------------------ */
  ...
  #if LV_COLOR_16_SWAP
    #define GD_PUT_565(dst16, c16)      /* big-endian (MSB first) */ \
        do { (dst16)[0] = (uint8_t)((c16) >> 8); \
             (dst16)[1] = (uint8_t)(c16); } while(0)
  #else
    #define GD_PUT_565(dst16, c16)      /* little-endian (LSB first) */ 
        do { (dst16)[0] = (uint8_t)(c16); 
             (dst16)[1] = (uint8_t)((c16) >> 8); } while(0)
  #endif
#endif
...
static gd_GIF * gif_open(gd_GIF * gif_base)
{
    ...
    for (i = 0; i < gif->width * gif->height; i++) {
        ...
        GD_PUT_565(&gif->canvas[i*3], c.full);
    ...
    for (j = 0; j < gif->fh; j++) {
        for (k = 0; k < gif->fw; k++) {
            ...
            if (!gif->gce.transparency || index != gif->gce.tindex) {
                ...
                GD_PUT_565(&buffer[(i+k)*3], c.full);
                buffer[(i+k)*3 + 2] = 0xff;
#elif LV_COLOR_DEPTH == 8
                lv_color_t c = lv_color_make(*(color + 0), *(color + 1), *(color + 2));
    ...
    case 2: /* Restore to background color. */
        ...
        for (j = 0; j < gif->fh; j++) {
            for (k = 0; k < gif->fw; k++) {
                ...
                GD_PUT_565(&gif->canvas[(i+k)*3], c.full);
