 ...
 *      INCLUDES
 *********************/
#pragma message "[OVERRIDES] Using patched lv_gif.c"
...
static void next_frame_task_cb(lv_timer_t * t)
{
    ...
    uint32_t delay = gifobj->gif->gce.delay;
    // Enforce a minimum delay to prevent too-fast rendering
    if(delay < 5) delay = 5;  // Minimum 50 ms
